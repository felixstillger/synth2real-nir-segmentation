...
parser.add_argument('--grayscale-style', action='store_true',
                    help='If set, style images are grayscaled before style transfer.')
parser.add_argument('--grayscale-content', action='store_true',
                    help='If set, content images are grayscaled before style transfer.')
# random.seed(131213)
    ...
    with tqdm(total=len(content_paths)) as pbar:
        for content_path in content_paths:
            # if str(content_path) == "/home/bhamscher/datasets/PASCALVOC/VOCdevkit/VOC2010/JPEGImages_3_split_cropped/train/2008_003947.jpg":
                try:
                    if not args.grayscale_content:  
                        content_img = Image.open(content_path).convert('RGB')
                    else:
                        content_img = Image.open(content_path).convert('L').convert('RGB')
                    for style_path in random.sample(styles, args.num_styles):
                        if not args.grayscale_style:
                            style_img = Image.open(style_path).convert('RGB')
                        else:
                            style_img = Image.open(style_path).convert('L').convert('RGB')
